    ...
    @foreach($brands->filter(fn($brand) => strtoupper($brand->label[0]) == $letter) as $brand)
        @if($brand->count)
            ...
            <li class="flex w-1/2 sm:w-1/6 px-2 mb-2">
                <a href="{{ url(!empty($brand->url_alias) ? $brand->url_alias : strtolower(str_replace(' ', '_', $brand->label))) }}" class="flex flex-1 w-full items-center justify-center border rounded p-3 hover:border-primary transition duration-150 ease-in-out overflow-hidden h-[130px] md:h-[170px]">
                    @if($brand->image)
